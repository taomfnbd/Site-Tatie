...
export const ContentProvider: React.FC<{ children: React.ReactNode }> = ({
    ...
    () => {
      try {
        ...
        if (saved) {
          ...
          const mergedHeader = {
            ...initialSiteContent.header,
            ...(parsed.header || {}),
          };
          // Réconcilier les liens de nav par `href` : la liste des pages vient
          // toujours de initialSiteContent (à jour), mais on conserve les noms
          // éventuellement renommés via le CMS (stockés dans localStorage).
          mergedHeader.links = initialSiteContent.header.links.map((link) => {
            const saved = (parsed.header?.links || []).find(
              (l) => l.href === link.href,
            );
            return saved ? { ...link, name: saved.name } : link;
          });
          return {
            ...initialSiteContent,
            ...parsed,
  ...
  React.useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      const hasChanges =
        Object.values(pagesContent).some((page) => page.hasChanges) ||
        globalHasChanges;
      if (hasChanges) {
        e.preventDefault();
        e.returnValue =
  ...
  const updatePageSections = useCallback(
    (pageKey: string, sections: Section[]) => {
  ...
  const getGlobalContent = useCallback(
    (key: string) => {
