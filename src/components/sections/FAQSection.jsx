...
const FAQSection = ({ content, onUpdate }) => {
  ...
  const data = {
    ...
    intro: 'Réponses aux questions les plus courantes avant de prendre rendez-vous.',
    items: [
      {
        question: 'Comment se déroule une séance ?',
        answer: 'La séance commence par un échange sur vos besoins, votre rythme de vie et vos attentes. Les conseils proposés sont personnalisés et adaptés à votre situation.'
      }
    ],
    ...content
  };
  ...
  const addItem = () => {
    onUpdate({
      ...data,
      items: [...data.items, { question: 'Nouvelle question', answer: 'Nouvelle réponse.' }]
    });
  };
    ...
    >
      <div className="mx-auto max-w-3xl text-center mb-10">
        <EditableText
          ...
          element="h2"
          className="text-2xl sm:text-3xl font-light text-stone-800 mb-4 tracking-wide"
        />
        <EditableText
          value={data.intro}
          onChange={(val) => updateField('intro', val)}
          element="p"
          className="text-sm sm:text-base text-stone-600 font-light leading-relaxed max-w-2xl mx-auto"
        />
      </div>
      ...
      <div className="mx-auto max-w-4xl space-y-3">
        {data.items.map((item, index) => {
          ...
          <div
            ...
            className={`group rounded-xl border bg-white transition-all duration-200 ${
              isOpen
                ? 'border-[#95a58d]/40 shadow-md shadow-stone-200/70'
                : 'border-stone-100 shadow-sm hover:border-[#95a58d]/25 hover:shadow-md hover:shadow-stone-200/60'
            }`}
          >
            <div className="flex items-start gap-3 p-4 sm:p-5">
              <span
                className={`mt-0.5 flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full text-xs font-medium transition-colors ${
                  isOpen ? 'bg-[#95a58d] text-white' : 'bg-stone-50 text-[#7a8471]'
                }`}
                aria-hidden="true"
              >
              ...
              {isEditMode ? (
                ...
                <h3 className="flex-1">
                  ...
                  >
                    ...
                    <FiChevronDown
                      className={`mt-0.5 h-5 w-5 flex-shrink-0 text-[#95a58d] transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}
                      aria-hidden="true"
                    />
                  </button>
                </h3>
                ...
                >
                  <div className="px-4 pb-5 pt-4 sm:px-5 sm:pl-[4.25rem] sm:pr-12">
                    <EditableText
                      ...
                      element="p"
                      className="text-sm sm:text-base text-stone-700 leading-7 font-light"
                    />
                  </div>
                </motion.div>
              )}
        ...
        {isEditMode && (
          <div className="text-center pt-3">
            <button
              onClick={addItem}
              className="text-sm text-[#7a8471] hover:text-stone-800 border border-[#95a58d]/50 rounded-full px-4 py-2 hover:bg-white transition-colors"
            >
