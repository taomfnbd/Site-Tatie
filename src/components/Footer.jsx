  ...
  const handleInstagramClick = (e) => {
    ...
    if (
      ...
      try {
        ...
        if (
          confirm(
            `Voulez-vous copier le nom d'utilisateur Instagram ?\n\nRecherchez ensuite "@${footerData.social.instagram}" dans Instagram.`,
          )
        ) {
          if (navigator.clipboard) {
            navigator.clipboard
              .writeText(`@${footerData.social.instagram}`)
              .then(() => {
                alert(
                  `✅ Copié ! Recherchez "@${footerData.social.instagram}" dans Instagram`,
                );
              });
          }
        }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 relative z-10">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
              ...
              <div>
                <h3 className="text-base sm:text-lg font-semibold">
                  <EditableText
                    ...
                    onChange={(v) => updateField("brand.name", v)}
                    element="span"
                  />
                </h3>
                <p className="text-[#6b7563] text-xs sm:text-sm font-medium">
                  <EditableText
                    value={footerData.brand.role}
                    onChange={(v) => updateField("brand.role", v)}
                    element="span"
                  />
                </p>
              </div>
          ...
          <div className="text-center sm:text-left">
            <h4 className="font-semibold mb-3 sm:mb-4 text-gray-200 text-sm sm:text-base">
              Navigation
            </h4>
            <ul className="space-y-1 sm:space-y-2">
              <li>
                <Link
                  to="/"
                  className="text-gray-300 hover:text-[#6b7563] transition-colors text-xs sm:text-sm"
                >
                  Accueil
                </Link>
              </li>
          ...
          <div className="text-center sm:text-left">
            ...
            <ul className="space-y-2 sm:space-y-3">
              <li className="flex items-center justify-center sm:justify-start space-x-2 sm:space-x-3">
                <SafeIcon
                  icon={FiPhone}
                  className="text-[#6b7563] text-xs sm:text-sm flex-shrink-0"
                />
                <a
                  href={`tel:${footerData.contact.phone.replace(/\./g, "")}`}
              ...
              <li className="flex items-start justify-center sm:justify-start space-x-2 sm:space-x-3">
                <SafeIcon
                  icon={FiMapPin}
                  className="text-[#6b7563] text-xs sm:text-sm mt-0.5 flex-shrink-0"
                />
                <EditableText
              ...
              <li className="flex items-center justify-center sm:justify-start space-x-2 sm:space-x-3">
                ...
                >
                  <EditableText
                    value={footerData.contact.bookingText}
                    onChange={(v) => updateField("contact.bookingText", v)}
                    element="span"
                  />
                </a>
              </li>
          ...
          <div className="text-center sm:text-left">
            ...
            <div>
              <h4 className="font-semibold mb-2 sm:mb-3 text-gray-200 text-sm sm:text-base">
                Suivez-moi
              </h4>
              <div className="flex justify-center sm:justify-start">
                ...
                >
                  <SafeIcon
                    icon={FiInstagram}
                    className="text-xs sm:text-sm group-hover:scale-110 transition-transform"
                  />
                </button>
              </div>
            </div>
